...
Transform any codebase into a stunning, interactive course. The working output is a **directory** containing a pre-built `styles.css`, `main.js`, per-module HTML files, and the source shells. Running `build.sh` assembles a **self-contained `index.html`** — the CSS and JS are inlined, so this single file is portable: open it directly in the browser or share/email it on its own, no setup required. (Fonts load from Google Fonts when online and fall back to system fonts offline, so it stays readable with no network.) The course teaches how the code works through scroll-based modules, animated visualizations, embedded quizzes, and plain-English translations of code.
...
The directory-based workflow is intentional: separating CSS/JS from content means AI never regenerates boilerplate, and each module is written independently (keeping output size small and quality high). At build time `build.sh` inlines the CSS and JS into a single `index.html`, so the deliverable the user opens or shares is one portable file that runs with zero setup.
...
```
This produces a self-contained `index.html` (CSS and JS inlined). Open it in the browser — or hand the user that single file to share.
